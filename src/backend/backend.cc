...
extern void InstrSelect(ir::Module &m, Asm &_asm);
extern void RegAlloca(Asm &_asm);

void IrToAsm(ir::Module &m, Asm &_asm, bool disable_ra) {
    InstrSelect(m, _asm);
    if (!disable_ra)
        RegAlloca(_asm);
}
...
Reg Op2Reg(Func &func, instr_list &insts, instr_iter it, Operand op, Reg rd) {
    if (op.flags & op.kIsImm) {
        ...
        if (nbit <= 16) {
            ...
            Load(insts, it, rd, Address(label));
            // Load(insts, it, rd, rd);
            return rd;
        }
    } else {
